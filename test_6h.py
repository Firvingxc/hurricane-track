...
# data_track=pd.read_csv(r'./test2020.txt',sep=',',header=None,names=['name','date','lat','lon','ws','p','speed','direct'])
# index_i=[]
...
# data_track.to_csv(r'./test2020.txt',index=False,header=False)
# 环境数据 - 替换成预测数据
# #
...
# #
your_data = xr.open_dataset('D:\latlon\coverland\du1_swh2017-2020.nc')
lat_swh = your_data['latitude'].values
print(lat_swh)
lon_swh = your_data['longitude'].values
print(lon_swh)
ds_u10 = xr.open_dataset('D:\latlon\coverland\du1_uv2017-2020_24znew_6.nc')
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.fc = nn.Linear(1 * 9 * 15, 100)  # Output 2 coordinates
        self.fc1 = nn.Linear(100, 2)  # Output 2 coordinates
...
class UNet(nn.Module):
    def __init__(self, n_channels, n_classes, bilinear=True):
        ...
        self.fc = nn.Linear(n_classes * 9 * 15, 2)  # Output 2 coordinates
...
#model = ConvNet()
model = UNet(3,1)
model.load_state_dict(torch.load('D:\latlon\coverland/best_model3t_ORIGMSE3.pth'))
model.eval()
#预报

data_track = pd.read_csv(r'D:\latlon\coverland/test20202.txt', sep=',', header=None,
...
for i in range(len(index_i)):
    ...
    while j <=end1-2:
        ...
        #ds_u10 = xr.open_dataset('du1_uv2017-2020_12znew.nc').sel(latitude=lat_swh, longitude=lon_swh)
        ds_u10 = xr.open_dataset('D:\latlon\coverland/du1_uv2017-2020_24znew_6.nc').sel(latitude=lat_swh, longitude=lon_swh)

        # if count == 8:
        #     your_data = xr.open_dataset('du1_swh2017-2020_48.nc')
        #     ds_u10 = xr.open_dataset('du1_uv2017-2020_48new.nc').sel(latitude=lat_swh,
        #                                                           longitude=lon_swh)
        #
        # if count == 6:
        ...
        lat_min = your_data['latitude'].values[lat_idx] - 4
        lat_max = your_data['latitude'].values[lat_idx] + 4
        lon_min = your_data['longitude'].values[lon_idx] - 7
        lon_max = your_data['longitude'].values[lon_idx] + 7
        # new
        # lat_min =lat-2
        # lat_max = lat+2
        # lon_min = lon-2
        ...
        if data.shape[0] != 9 or data.shape[1] != 15:
            # 创建一个新的17x17数组，用0填充
            new_data = np.zeros((9, 15, data.shape[2]))

            new_data[n:data.shape[0] + n, m:data.shape[1] + m, :] = data

            data = new_data
        if data.shape[0] == 9 and data.shape[1] == 15:
            X.append(data)
        ...
        One_p = data
        scaler = joblib.load('D:\latlon\coverland/scaler.pkl')

        One_p = One_p.reshape(-1, 3)
        One_p = scaler.transform(One_p)
        One_p = One_p.reshape(1, 9, 15, 3)
        ...
        for u in range(count):
            if j < end1 - 1:
                ...
                if 7>u>=3:
                    print('48,u',u)
                    your_data = xr.open_dataset('D:\latlon\coverland/du1_swh2017-2022_day2.nc')
                    ds_u10 = xr.open_dataset('D:\latlon\coverland/du1_uv2017-2020_48day2.nc').sel(latitude=lat_swh,
                                                                          longitude=lon_swh)

                if u>=7:
                    print('72,u', u)
                    your_data = xr.open_dataset('D:\latlon\coverland/du1_swh2017-2022_day3.nc')
                    ds_u10 = xr.open_dataset('D:\latlon\coverland/du1_uv2017-2020_72day3.nc').sel(latitude=lat_swh, longitude=lon_swh)
...
# # 将DataFrame保存为TXT文件
# df_targets.to_csv('coodi_cnntruth72.txt', sep=' ', index=False, header=False)
# df_predictions.to_csv('coodi_cnnpred72.txt', sep=' ', index=False, header=False)
# #
# # print("Data saved successfully.")
# #
# np.save('coodi_cnntruth72.npy',targets_denorm)
# np.save('coodi_cnnpred72.npy',predictions_denorm)
